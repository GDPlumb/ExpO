...
def eval(X_train, y_train, X_val, y_val, X_test, y_test, regularizer = "None", name = "tb"):
    ...
    if regularizer != "None":
        # Parameters
        c = tf.constant(100.0)
        num_points = 1 #Number to use to approximate the regularizer per minibatch
    ...
    if regularizer == "Causal":
        ...
        def regularizer(x):
            with tf.name_scope("CausalRegularizer") as scope:
                ...
                # Duplicates each row of x num_samples times, adds random noise and a constant term, and returns in a shape of [None, num_samples, n_input + 1]
                def generate_neighborhood(x):
                    ...
                    return tf.concat([x_expanded + noise, constant_term], 1)
                def x_tensor(x):
                    return tf.map_fn(generate_neighborhood, x)
                x_t = x_tensor(x)
                
                # Computes the project matrix for each of the linear regression problems stored along the first dimension of x_t
                def compute_projection_matrix(x):
                    return tf.matmul(tf.matrix_inverse(tf.matmul(tf.transpose(x), x)), tf.transpose(x))
                def p_tensor(x):
                    return tf.map_fn(compute_projection_matrix, x)
                p_t = p_tensor(x_t)
                
                # Compute the MSE of each of the linear regression problems
